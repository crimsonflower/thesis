...
The task of correctly identifying background is therefore crucial in the process of searches for new physics events. Monte Carlo (MC) simulations are used to generate a sample according to a probability distribution of a particular process. In this thesis MC simulated data samples are used to model background events along with SUSY signal events. All the samples include events that result in dileptonic decays. 
...
\subsection{Variables used in searches for supersymmetry}
